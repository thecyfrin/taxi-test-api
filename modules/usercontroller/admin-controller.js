...
module.exports = {
	createAdmin: async (req, res) => {
		try {
			...
			const jwtToken = jwt.sign(tokenObject, process.env.MODERATOR_SECRET, {
				expiresIn: "1d",
			});
			...
			if (response.isModified) {
				...
				return res
					.status(500)
					.json({ success: false, data: "updating-refresh-token-error" });
			}
		} catch (error) {
	...
	loginAdmin: async (req, res) => {
		try {
			...
			const isPassEqual = await compareHashData(password, admin.password);
			...
			if ((admin.canRead && admin.canWrite && admin.canExecute) == true) {
				...
				admin.canRead == true &&
				(admin.canWrite == true || admin.canExecute == true)
			) {
	...
	addNewAdmin: async (req, res) => {
		try {
			...
			return res
				.status(201)
				.json({ success: true, message: "invitation-sent" });
		} catch (error) {
			return res.status(500).json({ success: false, data: error });
		}
	},
	...
	getDriverManagementData: async (req, res) => {
		try {
			...
			const pending = await DriverModel.countDocuments({
	...
	modifySubscription: async (req, res) => {
		try {
			...
			if (response.isModified) {
				return res
					.status(201)
					.json({ success: true, message: "subscription-modified" });
			} else {
				return res
					.status(500)
					.json({ success: false, data: "updating-subscription-token-error" });
			}
		} catch (error) {
			return res.status(500).json({ success: false, data: error });
		}
	...
	approveDriver: async (req, res) => {
		try {
			...
			const vehicle = driver.vehicleDetails.find((v) => v.id === vehicleId);
			...
			return res
				.status(200)
				.json({ success: true, message: "driver-accepted" });
		} catch (error) {
			return res.status(500).json({ success: false, data: error });
		}
	},
	...
	rejectDrivers: async (req, res) => {
		try {
			...
			if (result.isModified) {
				return res
					.status(200)
					.json({ success: true, message: "driver-rejected" });
			} else {
				return res
					.status(500)
			...
			const reviews = await ReviewModel.aggregate([
				...
				},
				{
					$project: {
						...
						driverReview: {
							$ifNull: [{ $arrayElemAt: ["$driver.ratingStar", 0] }, 0.5],
						},
						createdAt: 1,
					},
				},
	...
	updateReview: async (req, res) => {
		try {
			...
			if (!review) {
				return res
					.status(401)
					.json({ success: false, data: "review-not-found" });
			}
			...
			if (result.isModified) {
				return res
					.status(200)
					.json({ success: true, message: "review-updated" });
			} else {
				return res
					.status(500)
					.json({ success: false, message: "failed-to-update-database" });
	...
	deleteReview: async (req, res) => {
		try {
			...
			if (review) {
				...
				return res
					.status(410)
					.json({ success: false, data: "delete-review-failed" });
			}
		} catch (error) {
			return res.status(500).json({ success: false, data: error });
		}
	...
	getTransactions: async (req, res) => {
		try {
			...
			if (transaction) {
				return res.status(200).json({
					success: true,
					tripTransactionAmount: total.toFixed(2),
					tripTransactions: tripTrans,
					withdrawTransactionAmount: withdrawal.toFixed(2),
					withdrawTransactions: withdrawTrans,
				});
			} else {
				return res
					.status(410)
	...
	getAnalyticsByCountry: async (req, res) => {
		try {
			...
			const completedTrips = await TripModel.countDocuments({
				country: country,
				isTripFinished: true,
			});
			const cancelledTrips = await TripModel.countDocuments({
				country: country,
