...
module.exports = {
    ...
    registerDriver: async (req,res) => {
        ...
        try {
            ...
            sendDriverOtp({ email: req.body.email, firstName: req.body.firstName, otpCode: generatedOtp });
    ...
    completeDriverRegistration: async (req,res) => {
        try {
            ...
            const tokenObject = {
                ...
                email : driver.email,
                profilePicture: driver.driverDetails.profilePicture,
    ...
    loginDriver: async (req,res) => {
        try {
            ...
            const isPassEqual = await compareHashData(req.body.password, driver.password);
            if(!isPassEqual) {
            ...
            const refreshToken = jwt.sign(tokenObject, process.env.REFRESH_SECRET, {expiresIn: '5d'});
    ...
    changeDriverPassword: async (req, res) => {
        try{
            ...
            const isPassEqual = await compareHashData(req.body.oldPassword, driver.password);
    ...
    refreshDriverToken: async (req, res) => {
        try {
            ...
            const refreshTokenEqual = req.body.refreshToken === driverModel.refreshToken;
    ...
    verifyDriverOtp: async (req, res) => {
        try {
            ...
            if(!driver) {
                return res.status(401).json({ success : false, message: 'email-not-found', data: "Email not registered!" });
            }
    
            const isOtpEqual = await compareHashData(req.body.otpCode, driver.otpCode);
    
            if(!isOtpEqual) {
                return res.status(401).json({ success : false, message: 'wrong-otp', data: "Enter correct OTP" });
            }
